...
bool MovementStraight::update()
{
    ...
    if (m_time >= 100) {
        ...
        m_obj->x = m_start_x + offset_x * (m_time * Level::time_slice / duration);
        m_obj->y = m_start_y + offset_y * (m_time * Level::time_slice / duration);
        m_obj->invalidate_view();
        return true;
    }
}
...
bool MovementRoll::update()
{
    ...
    if (m_time >= 50) {
        m_obj->x = m_start_x + offset_x;
        m_obj->y = m_start_y + offset_y * ((m_time-50) * Level::time_slice * 2);
        _cleared_from = true;
        m_from->reserved_by = nullptr;
    } else {
