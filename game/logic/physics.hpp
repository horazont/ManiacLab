...
class ILabSim {
public:
    static const SimFloat flow_friction;
    static const SimFloat flow_damping;
    static const SimFloat convection_friction;
    static const SimFloat heat_flow_friction;
    static const SimFloat fog_flow_friction;
...
public:
    ...
    inline const LabCell &front_cell_at(CoordInt x, CoordInt y)
    {
    ...
    inline const LabCell *safe_front_cell_at(CoordInt x, CoordInt y)
    {
...
public:
    NativeSimWorker(
        NativeLabSim &data_class,
        CoordInt slice_y0,
        CoordInt slice_y1);
    ~NativeSimWorker();

private:
    NativeLabSim &m_dataclass;
    const CoordInt m_width, m_height, m_slice_y0, m_slice_y1;
    const SimulationConfig m_sim;
    std::vector<LabCell> &m_back_cells, &m_front_cells;
    std::vector<LabCellMeta> &m_meta_cells;
    std::atomic_bool m_terminated;
    std::thread m_thread;

protected:
    void activate_cell(LabCell *front, LabCell *back);
    ...
    double flow(
        const LabCell *b_cellA,
        LabCell *f_cellA,
        const LabCell *b_cellB,
        LabCell *f_cellB,
        CoordInt direction);

    void temperature_flow(
        const LabCellMeta *m_cellA,
        const LabCell *b_cellA,
        LabCell *f_CellA,
        const LabCellMeta *m_cellB,
        const LabCell *b_cellB,
        LabCell *f_cellB,
        CoordInt direction);
